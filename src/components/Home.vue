...
function findApprovedVideoFiles() {
  ...
  Axios.all([axios.get("videos4carousel")])
    .then((response) => {
      ...
      if(state.videos4carousel.length>=40){
        if( addDays(new Date(), -7) > new Date(state.videos4carousel[0].datetime) ){
          ...
          state.mainTitle = "APPROVED VIDEOS - Last 40. Search for more.";
        }
      }
...
<template>
  ...
  <el-row>
    <el-col :span="24" max-height="700" >
      <carousel :itemsToShow="2.6" :wrapAround="true" :transition="500" >
        <slide v-for="video in state.videos4carousel" :key="video" style="margin-bottom:2em">
          <div id="sliderdiv1">
            <!-- <div>
          ...
          <div class="carousel__item">
            ...
            <div style="width:500px; max-height:200px; overflow:auto; font-size:0.6em; padding-bottom:2em; margin-bottom:2em">
                <p>{{ video.abstract }}</p>
              <el-link :href="video.ytUrl" style="margin-right:1em">YouTube video</el-link>
                <el-link :href="'uploads/' + video.pdfFileGuid" >METADATA</el-link >              
            </div>
          </div>
          <!-- <div id="sliderdiv2">
            <div>
